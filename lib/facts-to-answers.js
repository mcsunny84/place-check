...
function factsToAnswers(facts, opts) {
  ...
  // A1
  {
    ...
    const priv = obs(facts, 'phonePrivate');
    if (nonBlank(p.value) && isVal(p) || nonBlank(v.value) && isVal(v)) set('A1', 'yes', 'phone|virtualPhone');
    else if (isVal(priv) && priv.value === true) { set('A1', 'yes', 'phoneInfo.isPrivatePhone'); auto.A1.note = '등록됐지만 비공개 설정'; }
  ...
  // A2 · A3
  {
    ...
    else if (hours && hours.length >= 1 && hours.length < 7) set('A3', 'yes', 'businessHours.days'); // 요일이 빠짐 = 정기휴무 등록
    else if (hours && hours.length >= 7) { set('A3', 'na', 'businessHours'); auto.A3.note = '7일 영업·휴게 없음으로 등록됨'; }
    else unk('A3', 'businessHours.breaks', 'missing');
  }
  // A4
  {
    const r = obs(facts, 'road');
    if (isVal(r) && nonBlank(r.value)) set('A4', 'yes', 'road');
    else if (isEmpty(r) || (isVal(r) && !nonBlank(r.value))) set('A4', 'no', 'road'); // 필드는 항상 존재(3매장 실측) → 빈 값 = 미등록
    else unk('A4', 'road', 'missing');
  }
  // B1
  {
  ...
  // C3
  {
    const cp = obs(facts, 'coupons');
    const co = obs(facts, 'hasCouponCount');
    const count = isVal(cp) && cp.value && Array.isArray(cp.value.coupons) ? cp.value.coupons.length : isVal(co) ? Number(co.value) : isEmpty(co) ? 0 : null; // GraphQL 쿠폰 목록 우선
    const events = feeds ? feeds.filter(f => f && f.category === 'EVENT') : [];
    // start ≤ asOf ≤ end (daysAgo는 유효 날짜·순서 둘 다 검사)
    // 기간 있음: start ≤ asOf ≤ end. 기간 없는 EVENT 게시물: 최근 90일 내 작성이면 진행 중으로 간주(네이버가 기간 없이도 이벤트로 노출)
    const ago = f => S.daysAgo(f.createdAt, asOf);
    const active = f => (f.periodStart || f.periodEnd) ? (S.daysAgo(f.periodStart, asOf) !== null && S.daysAgo(asOf, f.periodEnd) !== null) : (ago(f) !== null && ago(f) <= 90);
    ...
    else if (!feeds) unk('C3', 'feeds', 'feed 탭 미수집'); // 첫 페이지(최신순)에 진행 중 EVENT 없음 → no (B6와 같은 근거)
  ...
  if (val(facts, 'visitorReviewsTotal') === 0) {
    ...
    const latestAny = val(facts, 'latestReviewDate') || null; // 180일 창 밖 리뷰까지 본 최신 방문일(server collectReviews)
    if (Array.isArray(list) && list.length) {
      const latestVisit = maxDate([latestAny, ...list.map((r) => r.visited || r.created)]);
      ...
      set('D1', latestAny, 'latestReviewDate'); // 최근 6개월엔 리뷰 없지만 더 오래된 리뷰는 있음
      unk('D2', 'reviews', '최근 6개월 리뷰 없음 → 답글 최신일 확인 불가');
    } else {
  ...
  // D3
  {
    ...
    if (text.trim() && S.scanDanger(text).length) set('D3', { text }, 'description+feeds.desc'); // 검출 우선: 일부 missing이어도 읽힌 원문에 위험 표현이 있으면 fail
    else if (known(d) && known(fo)) set('D3', text.trim() ? { text } : 'clean', 'description+feeds.desc');
