...
function factsToAnswers(facts, opts) {
  ...
  const complete = val(facts, 'feedsComplete') === true;
  {
    if (!feeds) unk('B6', 'feeds', 'feed 탭 미수집');
      ...
      else if (nd <= 30 || complete) set('B6', newest, 'feeds.createdAt');
      else unk('B6', 'feeds.createdAt', '31일+ but incomplete');
    }
  }
  // B7
  {
  ...
  if (val(facts, 'visitorReviewsTotal') === 0) {
    ...
    unk('D1', 'visitorReviewsTotal', '최신 리뷰 날짜 SSR 없음 → 보충 질문');
    unk('D2', 'latestOwnerReplyDate', 'SSR 없음 → 보충 질문');
  }
  // D3
  {
    const d = obs(facts, 'description');
