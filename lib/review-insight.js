...
const MODEL = process.env.REVIEW_MODEL || 'claude-sonnet-5'; // 3모델 비교(2026-09-14): opus 가장 구체, sonnet 근접·5배 저렴·2배 빠름, haiku 두루뭉술·일부 오인
  ...
  if (call && hasBodies) {
    try {
      ...
      if (j && Array.isArray(j.strengths) && Array.isArray(j.improvements)) {
        out.source = 'llm';
        out.llm = { strengths: j.strengths.slice(0, 5), improvements: j.improvements.slice(0, 5), model: j.model || MODEL };
      }
