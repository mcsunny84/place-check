...
(function (root, f) { if (typeof module === 'object' && module.exports) module.exports = f(); else root.PlaceKeyword = f(); })(this, function () {
  ...
  function diagnoseKeywords(keywords, ctx) {
    ...
    var regionToks = [ctx.district, ctx.district && String(ctx.district).replace(/^(.{2,})동$/, '$1'), ctx.station, ctx.station && stationDisplay(ctx.station), ctx.station && String(ctx.station).replace(/역$/, '')]
  ...
  function recommendKeywords(ctx) {
    ctx = ctx || {};
    var d = ctx.district ? String(ctx.district).trim() : '';
    if (len(d) > 2 && /동$/.test(d)) d = d.slice(0, -1); // '안국동' → '안국' (v0.4 §2.5 예시)
  ...
  return { extractDistrict: extractDistrict, stationDisplay: stationDisplay, normalizeCategory: normalizeCategory, menuForKeyword: menuForKeyword, diagnoseKeywords: diagnoseKeywords, recommendKeywords: recommendKeywords };
});
