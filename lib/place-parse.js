...
function parseHome(html, { now } = {}) {
  ...
  return {
    ...
    hasFeed: obs(get(pd, 'hasFeed.feedExist'), 'boolean'), // 홈: 소식 존재 여부 — 소식 0건 매장은 feed 탭에 feeds 키가 없어 이걸로 확정
...
function parseFeed(html, facts) {
  ...
  if (!fr || typeof fr !== 'object') {
    const hf = facts.hasFeed;
    if (hf && hf.status === 'value' && hf.value === false) return { ...facts, feeds: EMPTY, feedsComplete: { status: 'value', value: true } }; // 소식 없음 확정
    return { ...facts, feeds: MISSING, feedsComplete: MISSING };
  }
...
function reviewDate(s, now) {
  ...
  if (!m) return null;
  const base = new Date((now ? new Date(now).getTime() : Date.now()) + 9 * 3600000); // KST 달력일(서버 TZ 무관): +9h 후 UTC getter
  let y = m[1] ? (m[1].length === 2 ? 2000 + Number(m[1]) : Number(m[1])) : base.getUTCFullYear(); // '25.12.5.금' 형식(2자리 연도)
  ...
  const iso = ymd(y, String(mo).padStart(2, '0'), String(d).padStart(2, '0'));
  return new Date(iso + 'T00:00:00Z').getUTCDate() === d ? iso : null; // 2.31 같은 없는 날짜 거절
}
