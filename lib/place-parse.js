...
function reviewDate(s, now) {
  ...
  let y = m[1] ? Number(m[1]) : base.getFullYear();
...
function parseReviewVisitor(html, facts, { now } = {}) {
  ...
  for (const v of items) {
    const id = v.id || v.reviewId; if (!id || seen.has(id)) continue; seen.add(id);
    reviews.push({
      id: String(id),
...
module.exports = { extractApollo, parseHome, parseInformation, parseFeed, parsePhoto, parseReviewVisitor, reviewDate };
