...
- 방문자 리뷰는 최근 20건만 읽는다(더 과거는 별도 GraphQL 필요). 리뷰 인사이트의 문장 요약(장점 5·보완 5)은 `.env`의 `ANTHROPIC_API_KEY`가 있으면 켜진다(모델 `REVIEW_MODEL`, 기본 claude-opus-5, 매장당 1회 호출 후 24시간 캐시, 약 15~20초). 키가 없으면 규칙 기반 결과만 나온다.
